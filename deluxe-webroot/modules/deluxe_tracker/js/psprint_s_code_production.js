...
var s_account = 'deluxepsprintprod,deluxeglobalprod'
...
s.usePlugins=true
s.enableVideoTracking=false //set to true if video tracking should be enabled
s.brand='PsPrint';
s.siteprefix='PSP';
...
function s_doPlugins(s) {
	...
	//store the pre-getValOnce campaign var in prop16, for use with Unified Sources
	s.prop16 = s.campaign;
	...
	if(document.referrer=='' && !s.eVar3 && gpv_pn=='')
	...
	if((s.events.indexOf('prodView')!=-1) && (s.events.indexOf('event3')==-1))
		s.events=s.events+',event3';

	//sets eVar8 for alternate campaign expiration
	if(s.campaign && !s.eVar8)
		s.eVar8=s.campaign;
    ...
    //      s.eVar46 = gpv_pn;
    s.eVar46 = s.pageName      // now current page name
    //end fix
...
/********** VIDEO TAGGING CONFIGURATION BEGIN **********/
if(s.enableVideoTracking){
	s.loadModule("Media");
	s.Media.autoTrack=false;
	s.Media.trackVars="events,prop33,eVar43,eVar54,eVar55";
	s.Media.trackEvents="event50,event51,event52,event53,event54,event55,event56";
	s.Media.trackMilestones="25,50,75,100";
	s.Media.playerName="JW Player";
	s.Media.segmentByMilestones=true;
	s.Media.trackUsingContextData=true;
	s.Media.contextDataMapping={
	  "a.media.name":"eVar43,prop33",
...
/********** VIDEO TAGGING CONFIGURATION END **********/
